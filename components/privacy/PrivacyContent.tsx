...
import type { PrivacySection } from "@/lib/data/privacy";
import { privacyContact } from "@/lib/data/privacy";

type PrivacyContentProps = {
  sections: PrivacySection[];
};

export async function PrivacyContent({ sections }: PrivacyContentProps) {
          ...
          >
            ...
            {section.id === "contact" ? (
              <ul className="mt-6 space-y-3 text-sm text-text-muted">
                ...
                <li>
                  <a
                    href={privacyContact.instagramHref}
                    target="_blank"
                    rel="noopener noreferrer"
                    className="inline-flex items-center gap-2 transition-colors hover:text-primary-dark"
                  >
                    <AtSign className="h-4 w-4 shrink-0" aria-hidden="true" />
                    {privacyContact.instagram}
                  </a>
                </li>
                <li>
                  <Link
