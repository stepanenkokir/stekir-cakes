...
import { getTermsContent } from "@/lib/data/terms";
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const t = await getTranslations({ locale: loc, namespace: "terms" });
...
export default async function TermsPage({ params }: PageProps) {
  ...
  const { sections } = getTermsContent(locale as Locale);
      ...
      <div className="mx-auto max-w-4xl px-4 sm:px-6 lg:px-8">
        ...
        <section className="mt-12 rounded-2xl border border-border bg-surface p-8 text-center shadow-card sm:p-10">
          ...
          <div className="mt-6 flex flex-col items-center justify-center gap-3 sm:flex-row">
            <Button href="/catalog">{t("browseCakes")}</Button>
